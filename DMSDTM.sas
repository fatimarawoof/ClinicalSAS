...
data dm2  (drop=birthdtyy studyid1 race1 ethnic1 race1 sex1 siteid1);
	set dm1   (rename=(studyid=studyid1 ethnic=ethnic1 race=race1 sex=sex1 
		siteid=siteid1));
	studyid=studyid1;
	usubjid=compress(studyid1)||'-'|| strip(subject);
	subjid=strip(subject);

	if datapagename='Demographics' then
		domain='DM';
...
data sdtmdm;
	attrib STUDYID LABEL='Study Identifier' LENGTH=$ 10 DOMAIN 
		LABEL='Domain Abbreviation' USUBJID LABEL='Unique Subject Identifier ' 
		LENGTH=$ 18 SUBJID LABEL='Subject Identifier for the Study' LENGTH=$ 8 DTHDTC 
		LABEL='Date/Time of Death' DTHFL LABEL='Subject Death Flag' RFSTDTC 
		LABEL='Subject Reference Start Date/Time' LENGTH=$ 19 RFENDTC 
		LABEL='Subject Reference End Date/Time' LENGTH=$ 19 RFXSTDTC 
		LABEL='Date/Time of First Study Treatment' LENGTH=$ 19 RFXENDTC 
		LABEL='Date/Time of Last Study Treatment ' LENGTH=$ 19 RFICDTC 
		LABEL='Date/Time of Informed Consent' LENGTH=$ 19 SITEID 
		LABEL='Study Site Identifier' LENGTH=$ 3 BRTHDTC LABEL='Date/Time of Birth' 
		AGE LABEL='Age' AGEU LABEL='Age Units' SEX LABEL='Sex' RACE LABEL='Race' 
		LENGTH=$ 41 ETHNIC LABEL='Ethnicity' ARMCD LABEL='Planned Arm Code' 
		LENGTH=$ 20 ARM LABEL='Description Of Planned Arm' LENGTH=$ 60 COUNTRY 
		LABEL='Country' LENGTH=$ 3 ACTARM LABEL='Description of Actual Arm' 
		LENGTH=$ 60 ACTARMCD LABEL='Actual Arm Code' LENGTH=$ 20;
	set dm3;
	keep STUDYID DOMAIN USUBJID SUBJID SITEID SEX RACE ETHNIC AGE AGEU COUNTRY 
		BRTHDTC RFICDTC DTHDTC DTHFL RFSTDTC RFENDTC RFXSTDTC RFXENDTC ARM ARMCD 
		ACTARM ACTARMCD;
run;
